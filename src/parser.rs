...
fn positive_integer<'a>() -> Parser<'a, u8, i64> {
    (one_of(b"123456789") + one_of(b"0123456789").repeat(0..))
        .collect()
        .convert(str::from_utf8)
        .convert(|s| i64::from_str(s))
}
...
fn die<'a>() -> Parser<'a, u8, DiceExpression> {
    (positive_integer().opt() - one_of(b"dt") + positive_integer()).map(|(n, d)| {
        match n {
            // For example 2d8
            Some(i) if i > 1 => DiceExpression::Die(d).repeat(i).sum(),
            // 1d6 or d10
            Some(_) | None => DiceExpression::Die(d),
        }
    })
}
...
fn plus<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b'+') + call(expression)).map(|(e, f)| e.add(f))
}

//Difference
fn minus<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b'-') + call(expression)).map(|(e, f)| e.subtract(f))
}

//Multiply
fn multiply<'a>() -> Parser<'a, u8, DiceExpression> {
    ((die() | constant() | parenthesis()) - sym(b'*') + call(leaf)).map(|(e, f)| e.multiply(f))
}

//Less than
fn lt<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b'<') + call(expression)).map(|(e, f)| e.lt(f))
}

//Equal
fn eq<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b'=') + call(expression)).map(|(e, f)| e.eq(f))
}
//Greater than
fn gt<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b'>') + call(expression)).map(|(e, f)| e.gt(f))
}
// repeat expression n times
fn repeat_many<'a>() -> Parser<'a, u8, DiceExpression> {
    (positive_integer() - sym(b'.') + call(leaf)).map(|(i, e)| e.repeat(i))
}

// multiple expressions separated by whitespace
fn many<'a>() -> Parser<'a, u8, DiceExpression> {
    (leaf() - sym(b',') - space().opt() + call(expression)).map(|(u, v)| u.also(v))
}
...
fn expression<'a>() -> Parser<'a, u8, DiceExpression> {
    space().opt() * (many() | plus() | minus() | lt() | gt() | eq() | negative() | leaf())
        - space().opt()
}
